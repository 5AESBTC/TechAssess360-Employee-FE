  ...
  <div class="info ms-3 text-start">
    ...
    <div class="line"><strong>Vị trí:</strong> {{ profile.position }}</div>
    <div class="line"><strong>Bộ Phận:</strong> {{ profile.department }}</div>
 ...
 <!-- Right Menu -->
    ...
    <div class="col-md-8 right-menu p-4">
      ...
      <div class="evaluation-header text-start mb-4">
        <label class="fw-bold fs-4">Đánh giá quý 4 năm 2024 cho : {{ profile.name }} </label>
      </div>
      ...
      <form class="evaluation-form">
          ...
          <div v-for="(question, index) in performanceEvaluation.performanceQuestions" :key="index" class="question mb-3">
            ...
            <div class="options d-flex justify-content-around my-3">
              <div v-for="(option, optIndex) in question.options" :key="optIndex" class="form-check">
                <input type="radio" :id="'performanceOption' + index + optIndex" :name="'performance' + index" class="form-check-input" v-model="selectedValues[index]" :value="option.value" />
                <label :for="'performanceOption' + index + optIndex" class="form-check-label">{{ option.label }}</label>
              </div>
            </div>
            <div class="description">
              <textarea v-if="parseFloat(selectedValues[index]) >= 100" class="form-control" rows="3" placeholder="Nhận xét thêm"></textarea>
            </div>
          </div>
        </div>
        ...
        <div class="section mb-4">
          ...
          <textarea class="form-control" rows="3" placeholder="Nhận xét thêm"></textarea>
        </div>
        ...
        <div class="section mb-4">
          ...
          <div class="question mb-3">
            <label class="d-flex justify-content-between title">Thành tích cá nhân nổi bật nhất của bạn trong thời gian qua là:</label>
            <textarea class="form-control" rows="3" placeholder="Nhận xét thêm"></textarea>
          </div>
          <div class="question mb-3">
            <label class="d-flex justify-content-between title">Bạn cảm thấy mình đã đóng góp đủ cho sự phát triển của tổ chức không?</label>
            <textarea class="form-control" rows="3" placeholder="Nhận xét thêm"></textarea>
          </div>
        </div>
        ...
        <div class="section mb-4">
          ...
          <textarea class="form-control" rows="3" placeholder="Kết quả đạt được quý II"></textarea>
        </div>
        <div class="section mb-4">
          <h5>Mục tiêu quý III</h5>
          <textarea class="form-control" placeholder="Kết quả đạt được quý III" rows="3"></textarea>
        </div>

        <!-- Form Buttons -->
        <div class="form-buttons d-flex justify-content-center gap-4 mt-4">
          <button type="button" class="btn btn-secondary">Lưu</button>
...
export default {
  ...
  data() {
    return {
      ...
      sortOrder: 'asc',
      teamMates: [
        { name: 'Nguyễn Lê Quốc Huy', position: 'Manager',project: 'StudyArt' },
      ...
      performanceEvaluation: {
        ...
        skillsQuestions: [
        {
          ...
          {
            label: ' Bạn cảm thấy cần cải thiện kỹ năng nào để nâng cao hiệu suất công việc?',
            score: 15,
            options: [
            { label: '1', value: '1' },
              { label: '2', value: '2' },
        ...
        personalContributionsQuestions: [
          ...
          { label: 'Bạn cảm thấy mình đã đóng góp đủ cho sự phát triển của tổ chức không?' }
        ]
      },
